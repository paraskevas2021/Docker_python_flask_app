...
app = Flask(__name__)
app.config['SQLALCHEMY_DATABASE_URI'] = 'postgresql://postgres:mysecretpassword@my-postgres:5432/postgres'
...
def submit_form():
    ...
    if errors:
        ...
        # Επικύρωση αν υπάρχει ήδη ο πελάτης με τον ίδιο ΑΤ
...
def search_customer():
    ...
    if search_value:
        ...
        if customers:
            ...
            # Επιστροφή κενού πίνακα αν δεν βρεθούν αποτελέσματα
            return jsonify([])
    else:
        return jsonify({'error': 'Δεν παρέχεται αναζήτηση'}), 400


# Αλλάζουμε το δρομολόγιο εδώ σε update-customer
...
def update_customer():
    ...
    if customer_id:
        customer = Customer.query.get(customer_id)
        if customer:
            # Ελέγχουμε τα δεδομένα που προσπαθεί να ενημερώσει ο χρήστης
...
def delete_customer_view():
    ...
    with db.session() as session:
        ...
        if customer:
            # Διαγραφή σχετικών διευθύνσεων και αριθμών τηλεφώνου
